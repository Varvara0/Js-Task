...
function check(number, min, max){
    if(number < max || number > min || min < max ){
        console.log(true);
    }else{
        console.log(false);
    }
...
for(let properties in engineers){
    console.log('Заработная плата ' + properties + ' составляет ' + engineers[properties] + ' рублей');
}
...
for(let i = 0; i < numbers.length; i++){
    console.log('Индeксу ' + i + ' соответствует число ' + numbers[i]);
}
...
for (let i = 0; i < newArr.length; i++) {
  console.log(`${newArr[i].word} - ${newArr[i]['word'].length}`);
};